...
const TvShowDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [dataRes, relatedRes, videosRes] = await Promise.all([
          Api(`tv/${mid}?api_key=${process.env.API_KEY}&language=en-US`),
          Api(`tv/${mid}/similar?api_key=${process.env.API_KEY}&language=en-US&page=1`),
          Api(`tv/${mid}/videos?api_key=${process.env.API_KEY}&language=en-US`)
        ]);
  ...
  if (!data) return <p className="text-white text-center mt-10">TV Show not found</p>;
          ...
          <div className="xl:px-20">
            <div className="lg:flex justify-between">
              ...
              <div className="lg:w-7/12">
                <MovieDetailData data={data} videos={videos} className="mb-5 lg:mb-8" />
